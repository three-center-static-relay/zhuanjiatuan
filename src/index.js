...
const CAP={task_profiler:true,dynamic_models:true,company_diversity:true,experts:true,judge:true,openrouter_sdk:true,official_model_catalog:true,tools:false,web:false,duplicate_task_rejected:true,cancel_checked_between_calls:true,external_calls_inside_single_task_lock:true,sdk_retries_disabled:true,sdk_chat_request_wrapper:true};
...
async function call(env,model,messages,maxTokens){const client=new OpenRouter({apiKey:env.OPENROUTER_API_KEY});return client.chat.send({chatRequest:{model,messages,reasoning:{effort:"high"},temperature:0.2,stream:false,...(maxTokens?{maxCompletionTokens:maxTokens}:{})}},{retries:{strategy:"none"}})}
