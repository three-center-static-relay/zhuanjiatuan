import base,{CenterGate as BaseCenterGate} from "./dynamic-index.js";
...
async function validateRunResponse(r,env){
  ...
  const expertEmpty=experts.length===0||experts.some(a=>!String(a?.content||"").trim());
  const judgeRequired=Number(body?.judge_count||0)>0;
  const judgeEmpty=judgeRequired&&!String(body?.judge?.content||"").trim();
  if(!expertEmpty&&!judgeEmpty)return r;
  const code=expertEmpty?"EMPTY_EXPERT_OUTPUT":"EMPTY_JUDGE_OUTPUT",id=String(body?.task_id||"");
  if(id)await g(env,`/task/${encodeURIComponent(id)}`,"POST",{status:"failed",error:code,experts:null,judges:null,judge:null,finished_at:new Date().toISOString()},`task:${id}`).catch(()=>{});
  return json({ok:false,error:code,message:"Cloudflare dynamic route returned an empty expert-panel result; task is not accepted as completed",task_id:id||null},502);
}

async function selftest(env,ctx){
  if(!aiGatewayConfigured(env))return json({ok:false,error:"AI_GATEWAY_NOT_CONFIGURED",business_e2e:false,configured:false,message:"Cloudflare AI Gateway dynamic route is not configured"},503);
  const taskId=`selftest-expert-${crypto.randomUUID()}`,started=Date.now();
  const request=new Request("https://expert.internal/v1/run",{method:"POST",headers:{"content-type":"application/json","x-three-center-selftest":"1"},body:JSON.stringify({task_id:taskId,prompt:"Self-test only. Return exactly: 2 — 1+1=2. Do not add external facts.",model_count:2,max_tokens:256,timeout_seconds:120})});
  const raw=await base.fetch(request,env,ctx),r=await validateRunResponse(raw,env),body=await r.clone().json().catch(()=>null),models=Array.isArray(body?.models)?body.models:[],companies=Array.isArray(body?.companies)?body.companies:[],experts=Array.isArray(body?.experts)?body.experts:[],judge=body?.judge||null;
