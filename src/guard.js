...
async function mapNoReplacement(body,env,status){
  ...
  return json({...body,error:code,message:last==="UPSTREAM_TIMEOUT"?"Cloudflare dynamic-route model stage timed out":"Cloudflare dynamic route returned an empty model result"},http);
}
...
async function validateRunResponse(r,env){
  ...
  return json({ok:false,error:code,message:"Cloudflare dynamic route returned an empty expert result; task is not accepted as completed",task_id:id||null},502);
}
...
async function selftest(env,ctx){
  ...
  const raw=await base.fetch(request,env,ctx),r=await validateRunResponse(raw,env),body=await r.clone().json().catch(()=>null),models=Array.isArray(body?.models)?body.models:[],companies=Array.isArray(body?.companies)?body.companies:[],answers=Array.isArray(body?.answers)?body.answers:[],judge=body?.judge||null;
  const uniqueCompanies=companies.length===models.length&&new Set(companies.map(x=>String(x).toLowerCase())).size===models.length,modelPolicy=models.length===2&&models.every(modelAllowed)&&uniqueCompanies,expertNonempty=answers.length===1&&Boolean(String(answers[0]?.content||"").trim()),judgeNonempty=Boolean(String(judge?.content||"").trim()),completed=r.ok&&body?.ok===true&&body?.status==="completed",ok=completed&&modelPolicy&&expertNonempty&&judgeNonempty;
