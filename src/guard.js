...
const CANARY_ID="prod-canary-20260815-a9f7c2";
const CANARY_PATH="/v1/.canary/20260815-a9f7c2";
...
async function selftest(env,ctx,taskId=`selftest-expert-${crypto.randomUUID()}`){
  ...
  const request=new Request("https://expert.internal/v1/run",{method:"POST",headers:{"content-type":"application/json","x-three-center-selftest":"1"},body:JSON.stringify({task_id:taskId,prompt:"Self-test only. Return the integer 2 and one short sentence confirming that 1+1=2. Do not add external facts.",model_count:2,max_tokens:96,timeout_seconds:120,roles:["Arithmetic verifier"]})});
  const r=await base.fetch(request,env,ctx),body=await r.json().catch(()=>null);
  if(r.status===409)return json({ok:false,error:body?.error||"BUSY",business_e2e:false,configured:true,task_id:taskId,message:"Selftest already running or duplicate"},409);
  const models=Array.isArray(body?.models)?body.models:[],answers=Array.isArray(body?.answers)?body.answers:[],judge=body?.judge||null;
...
async function prodCanary(env,ctx){
  ...
  if(old.task){for(let i=0;i<24;i++){await new Promise(r=>setTimeout(r,250));const x=await g(env,`/task/${encodeURIComponent(CANARY_ID)}`);if(x.task?.canary_receipt)return json(x.task.canary_receipt,x.task.canary_receipt.ok?200:503)}return json({ok:false,error:"CANARY_IN_PROGRESS",task_id:CANARY_ID},409)}
  const r=await selftest(env,ctx,CANARY_ID),body=await r.json().catch(()=>null);
  if(r.status===409){for(let i=0;i<24;i++){await new Promise(r=>setTimeout(r,250));const x=await g(env,`/task/${encodeURIComponent(CANARY_ID)}`);if(x.task?.canary_receipt)return json(x.task.canary_receipt,x.task.canary_receipt.ok?200:503)}return json({ok:false,error:"CANARY_IN_PROGRESS",task_id:CANARY_ID},409)}
  const receipt={...body,canary:true,canary_id:CANARY_ID,public_trigger_one_shot:true};
