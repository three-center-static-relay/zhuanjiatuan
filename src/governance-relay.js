import { aiGatewayConfigured, aiGatewayRequestHeaders, dynamicChatEndpoint, dynamicRouteModel } from "./ai-gateway.js";
...
function rejectToolUse(payload) {
  ...
  if (structured || textualInvocation) throw Object.assign(new Error("AUXILIARY_TOOL_USE_FORBIDDEN"), { status: 502 });
}
  ...
  try {
    const response = await fetch(await dynamicChatEndpoint(env), {
      ...
      headers: {
        ...
        ...aiGatewayRequestHeaders(env, MODEL_TIMEOUT_MS, { expert_slot: "governance" })
      },
      body: JSON.stringify({
    ...
    if (new TextEncoder().encode(raw).length > MAX_UPSTREAM_BYTES) throw Object.assign(new Error("UPSTREAM_RESPONSE_TOO_LARGE"), { status: 502 });
    let payload = null;
    if (raw) {
      try { payload = JSON.parse(raw); } catch { throw Object.assign(new Error("UPSTREAM_BAD_JSON"), { status: 502 }); }
    }
    if (!response.ok) throw Object.assign(new Error("UPSTREAM_UNAVAILABLE"), { status: response.status || 502, details: redact(payload) });
    const model = String(response.headers.get("cf-aig-model") || payload?.model || "").trim();
    const provider = String(response.headers.get("cf-aig-provider") || "").trim();
    if (!model || !provider) throw Object.assign(new Error("DYNAMIC_ROUTE_METADATA_MISSING"), { status: 502 });
...
export async function runGovernanceRelay(request, env) {
  if (!aiGatewayConfigured(env)) {
    return json({ ok: false, error: "AI_GATEWAY_NOT_CONFIGURED", message: "Cloudflare AI Gateway token, gateway id, route, or account id is not configured" }, 503);
  }
  try {
    ...
    const routed = await callRoute(env, [
      { role: "system", content: `${suppliedSystem}\n\n${AUXILIARY_NO_TOOLS_SYSTEM}` },
      { role: "user", content: prompt }
    ], maxTokens);
    return json({
      ok: true,
      provider: routed.provider,
      selection: "cloudflare-dynamic-route",
      route_slot: "governance",
    ...
    return json({ ok: false, error: String(error?.message || "INTERNAL_ERROR"), web_gpt_fallback_required: true, model_tier: "route-policy", tool_access: "none" }, error?.status || 500);
  }
}
