...
const network=setupServer(
  http.post(CHAT_ENDPOINT,async({request})=>{chatCalls++;const b=await request.json(),prompt=promptOf(b),metadata=JSON.parse(request.headers.get("cf-aig-metadata")||"{}");assert.equal(b.model,"dynamic/expert-panel-v1");assert.equal(metadata.dynamic_route,"expert-panel-v1");assert.ok(routeModels[metadata.expert_slot]);assert.equal(request.headers.get("cf-aig-authorization"),"Bearer test-gateway-token");assert.equal(request.headers.get("cf-aig-skip-cache"),null);assert.equal(request.headers.get("cf-aig-collect-log"),null);assert.equal(request.headers.get("cf-aig-max-attempts"),"1");if(prompt.includes("FAIL_CHAT"))return HttpResponse.json({error:{message:"synthetic chat failure"}},{status:503});const headers={"cf-aig-model":routeModels[metadata.expert_slot],"cf-aig-provider":"openrouter"};if(prompt.includes("OVERSIZE_CHAT"))return HttpResponse.json({choices:[{message:{role:"assistant",content:"x".repeat(1600000)}}]},{headers});if(prompt.includes("HOLD_CHAT")&&chatCalls===1){holdEnteredResolve();await holdRelease}return HttpResponse.json({id:`chat-${chatCalls}`,model:routeModels[metadata.expert_slot],choices:[{index:0,message:{role:"assistant",content:"2. 1+1=2."},finish_reason:"stop"}],usage:{prompt_tokens:8,completion_tokens:5,total_tokens:13}},{headers})})
);
...
try{
  ...
  await reset();const publicRoutes=await Promise.all([post("/v1/run",{task_id:"public",prompt:"x"},"public"),post("/v1/status",{task_id:"x"},"public"),post("/v1/cancel",{task_id:"x"},"public"),post("/v1/selftest",{},"public")]);assert.equal(publicRoutes.filter(x=>x.status===403&&x.body?.error==="POLICY_DENIED").length,4);assert.equal(chatCalls,0);
  await reset();armHold();const first=run("expert-holder","HOLD_CHAT load test",2);await within(holdEntered,5000,"holder-chat-entry");const contenders=await within(Promise.all(Array.from({length:128},(_,i)=>run(`expert-contender-${i}`,"NORMAL",2))),15000,"unique-contenders");assert.equal(contenders.filter(x=>x.status===409&&x.body?.error==="BUSY").length,128);assert.equal(chatCalls,1);letGo();assert.equal((await within(first,8000,"holder-finish")).status,200);assert.equal(chatCalls,2);
  await reset();armHold();const dupFirst=run("expert-duplicate","HOLD_CHAT duplicate test",2);await within(holdEntered,5000,"dup-chat-entry");const dup=await within(Promise.all(Array.from({length:128},()=>run("expert-duplicate","NORMAL",2))),15000,"duplicate-contenders");assert.equal(dup.filter(x=>x.status===409&&x.body?.error==="DUPLICATE_TASK").length,128);assert.equal(chatCalls,1);letGo();assert.equal((await within(dupFirst,8000,"dup-finish")).status,200);
  ...
  await reset();const filtered=await run("filter-clamp","NORMAL",99);assert.equal(filtered.status,200);const models=filtered.body?.models||[];assert.equal(models.length,4);assert.equal(models.some(banned),false);assert.equal(new Set(models.map(x=>String(x).split("/")[0])).size,4);assert.equal(chatCalls,4);
  await reset();const burst=await waves(520,64,i=>post("/v1/run",{task_id:`rate-${i}`}),"rate-wave");assert.equal(burst.filter(x=>x.status===429&&x.body?.error==="RATE_LIMITED").length,320);assert.equal(burst.filter(x=>x.status===400&&x.body?.error==="INVALID_REQUEST").length,200);assert.equal(chatCalls,0);
  await reset();const hr=await server.fetch(internal("/v1/run"),{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({task_id:"huge",prompt:"x".repeat(70000)})});assert.equal(hr.status,413);assert.equal(chatCalls,0);
  ...
  console.log(JSON.stringify({ok:true,suite:"expert-extreme-stress",max_parallel:128,concurrency_contenders:128,duplicate_contenders:128,rate_total:520,health_total:512,tests:["public-execution-deny","dynamic-route-only","slot-metadata","duplicate-id","cancel-before-judge","no-worker-retry","failure-release","response-size-cap","model-policy","company-diversity","model-clamp","rate-no-external-call","body-limit","read-burst"]}));
