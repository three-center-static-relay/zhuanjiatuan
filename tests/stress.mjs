...
const network=setupServer(
  ...
  http.post("https://openrouter.ai/api/v1/chat/completions",async({request})=>{
    ...
    if(prompt.includes("FAIL_CHAT"))return HttpResponse.json({error:{message:"synthetic chat failure"}},{status:503});
    if(prompt.includes("HOLD_CHAT")&&chatCalls===1){holdEnteredResolve();await holdRelease}
    return HttpResponse.json({id:`chat-${chatCalls}`,model:b.model,choices:[{index:0,message:{role:"assistant",content:"2. 1+1=2."},finish_reason:"stop"}],usage:{prompt_tokens:8,completion_tokens:5,total_tokens:13}});
  })
);
...
try{
  ...
  await reset();armHold();const first=run("expert-holder","HOLD_CHAT load test",2);await within(holdEntered,5000,"holder-chat-entry");const contenders=await within(Promise.all(Array.from({length:64},(_,i)=>run(`expert-contender-${i}`,"NORMAL",2))),12000,"unique-contenders");assert.equal(contenders.filter(x=>x.status===409&&x.body?.error==="BUSY").length,64);assert.equal(catalogCalls,1,"only active task may touch OpenRouter model catalog while lock held");assert.equal(chatCalls,1,"only active task may enter paid chat while lock held");letGo();assert.equal((await within(first,8000,"holder-finish")).status,200);assert.equal(chatCalls,2,"one expert plus one judge only");

  await reset();armHold();const dupFirst=run("expert-duplicate","HOLD_CHAT duplicate test",2);await within(holdEntered,5000,"dup-chat-entry");const dup=await within(Promise.all(Array.from({length:64},()=>run("expert-duplicate","NORMAL",2))),10000,"duplicate-contenders");assert.equal(dup.filter(x=>x.status===409&&x.body?.error==="DUPLICATE_TASK").length,64);assert.equal(catalogCalls,1,"duplicate task ids must not refetch model catalog");assert.equal(chatCalls,1);letGo();assert.equal((await within(dupFirst,8000,"dup-finish")).status,200);
  ...
  await reset();const burst=await within(Promise.all(Array.from({length:220},(_,i)=>post("/v1/run",{task_id:`rate-${i}`}))),15000,"rate-burst");assert.equal(burst.filter(x=>x.status===429&&x.body?.error==="RATE_LIMITED").length,20);assert.equal(burst.filter(x=>x.status===400&&x.body?.error==="INVALID_REQUEST").length,200);assert.equal(catalogCalls,0,"invalid/rate-limited burst must not hit model catalog");assert.equal(chatCalls,0,"invalid/rate-limited burst must never enter paid chat");
