...
const network=setupServer(
  ...
  http.post(CHAT_ENDPOINT,async({request})=>{chatCalls++;const b=await request.json(),prompt=promptOf(b);assert.equal(request.headers.get("cf-aig-authorization"),"Bearer test-gateway-token");assert.equal(request.headers.get("cf-aig-skip-cache"),null);assert.equal(request.headers.get("cf-aig-collect-log"),null);assert.equal(request.headers.get("cf-aig-max-attempts"),"1");if(prompt.includes("FAIL_CHAT"))return HttpResponse.json({error:{message:"synthetic chat failure"}},{status:503});if(prompt.includes("OVERSIZE_CHAT"))return HttpResponse.json({choices:[{message:{role:"assistant",content:"x".repeat(1600000)}}]});if(prompt.includes("HOLD_CHAT")&&chatCalls===1){holdEnteredResolve();await holdRelease}return HttpResponse.json({id:`chat-${chatCalls}`,model:b.model,choices:[{index:0,message:{role:"assistant",content:"2. 1+1=2."},finish_reason:"stop"}],usage:{prompt_tokens:8,completion_tokens:5,total_tokens:13}})})
);
