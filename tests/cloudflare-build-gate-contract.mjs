...
assert.match(pkg.scripts?.["test:build-gate"]||"",/tests\/dynamic-panel-contract\.mjs/);
assert.match(pkg.scripts?.["test:build-gate"]||"",/tests\/dynamic-runtime\.mjs/);
assert.match(pkg.scripts?.["test:build-gate"]||"",/tests\/langgraph-brain-contract\.mjs/);
assert.equal(wrangler.name,"expert-worker");
assert.equal(wrangler.workers_dev,false);
assert.equal(wrangler.preview_urls,false);
assert.equal(wrangler.ai?.binding,"AI");
assert.deepEqual(wrangler.secrets?.required,["AI_GATEWAY_TOKEN"]);
assert.equal(wrangler.build,undefined);
console.log(JSON.stringify({ok:true,suite:"cloudflare-build-gate-contract",worker:wrangler.name,preview_creates_version_without_production_deploy:true,secret:"AI_GATEWAY_TOKEN",workers_ai_binding:true,dynamic_panel_v3:true}));
