...
const all=bytes.toString("utf8").split(/\r?\n/).filter(Boolean).map(JSON.parse);assert.ok(all.length>=2000,"DATASET_TOO_SMALL");
...
const token=s.token,batchId="pilot-0000",waveId="pilot-v3";
...
assert.equal(br.status,200,`BATCH_HTTP_${br.status}:${JSON.stringify(b)}`);assert.equal(b?.ok,true,JSON.stringify(b));assert.equal(b?.n,10,JSON.stringify(b));assert.ok(Array.isArray(b?.primary_models)&&b.primary_models.length===2,JSON.stringify(b));assert.notEqual(String(b.primary_models[0]).split('/')[0].toLowerCase(),String(b.primary_models[1]).split('/')[0].toLowerCase(),JSON.stringify(b));assert.ok(Number(b.completed)>=8,`LOW_COMPLETION:${JSON.stringify(b)}`);assert.ok(Number(b.parsed)>=8,`LOW_PARSE:${JSON.stringify(b)}`);
const cr=await fetch(`${BASE}/wave/commit`,{method:"POST",headers:{"content-type":"application/json","x-benchmark-token":token,accept:"application/json"},body:JSON.stringify({wave_id:waveId,batch_ids:[batchId]})});const c=await cr.json().catch(()=>null);assert.equal(cr.status,200,`COMMIT_HTTP_${cr.status}:${JSON.stringify(c)}`);assert.equal(c?.ok,true,JSON.stringify(c));assert.equal(c?.n,10,JSON.stringify(c));
console.log(JSON.stringify({ok:true,suite:"medmcqa-pilot-v3",paid_call:true,n:c.n,correct:c.correct,completed:c.completed,parsed:c.parsed,cost:c.cost,primary_models:b.primary_models,dataset_rows:all.length}));
