...
### Lane-pair transport shards

The Expert Worker keeps a global lane number for each actual participant. Governance assigns lanes to distinct model-owner companies, up to eight lanes total. Concrete Cloudflare Dynamic Routes are deliberately split into four small transport shards: lanes 1–2, 3–4, 5–6 and 7–8.

Each request selects its transport shard from `metadata.lane`; `stage`, `capability`, `depth` and `cost_mode` remain metadata for semantic orchestration, policy, telemetry and within-shard cost routing. The semantic task type does not duplicate the same underlying Cloudflare graph under multiple route names.

Each active shard contains at most two company lanes. This preserves global 2–8-company diversity while bounding each Cloudflare graph to the small topology already accepted in production testing. A task that requests a lane not backed by the current governed company universe fails closed instead of reusing another company's lane.
...
- no sevenfold duplication of the same large lane graph by semantic task label.
...
The route registry remains replaceable through governed configuration. The four source classes are stable boundaries; the models inside those classes are a continuously refreshed universe. The default registry uses four lane-pair transport shards, each with at most two company lanes.
